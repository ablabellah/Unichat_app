 ...
 ?>
    <div class="container col-md-10 col-sm-12 col-lg-9 rounded-0 d-flex justify-content-between">
        <div class="col-md-8 col-sm-12" style="max-width:93vw">
            ...
            showError('post_img');
            if(count($posts)<1){
                echo "<p style='width:100%' class='p-2 bg-white border rounded text-center my-3 col-12'>Welcome to UniChat! If you need any help, our virtual assistant UniBot is here for you. 
                Connect with others, follow someone new, and share your experiences by adding a new post. 
...
if (!empty($post['post_img'])) {
    $fileExtension = strtolower(pathinfo($post['post_img'], PATHINFO_EXTENSION));
    if (in_array($fileExtension, array('jpg', 'jpeg', 'png'))) {
        // Display image
        echo '<img src="assets/images/posts/' . $post['post_img'] . '" loading="lazy" class="" alt="...">';
    } elseif (in_array($fileExtension, array('mp4', 'avi', 'mov'))) {
        // Display video
        echo '<video controls>
                  <source src="assets/images/posts/' . $post['post_img'] . '" type="video/' . $fileExtension . '">
              </video>';
}
}
?>
...
if($post['post_text']){
    ?>
                ...
                <div class="card-body">
  <?=makeUrlsClickable($post['post_text'])?>
</div>
    <?php
}
                ?>
...
if(count($follow_suggestions)<1){
    echo "<p class='p-2 bg-white border rounded text-center'>You have successfully followed all users. Your account is now fully connected!</p>";
}
                ?>
